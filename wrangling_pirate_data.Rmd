...
df_pirate_features %>%
 dplyr::select(active_days, active_weeks, minutes_per_active_day, hours_per_week) %>%
 dplyr::filter((is.infinite(minutes_per_active_day) & 
            active_days != 0)) %>%
  count()
...
looking whether inf minutes_per_active_day and inf hours_per_week respectively co-occur
with active_days and active_weeks that are different than 0, and we see that not
```{r}

```

I actually want to quickly confirm that those three column are the only ones with
inf values, and we see that's the case
```{r}
#looking at each column name
for(c in colnames(df_ctl)){
  #saving the boolean of whether there are infinite values in that column
  val = length(which(sapply(df_ctl[{c}], function(x) any(is.infinite((x)))))) > 0
...
So I'll change the mean_convo_len_min, minutes_per_active_day, and hours_per_week
to 0 where it's inf
Rerunning the code above shows this worked
```{r}
