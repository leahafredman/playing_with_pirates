...
---
I want to create a dataset with attributes of fictional pirates. 

Note that I am aware that this article has a ridiculous premise, and I did so because producing this type of content (that is, demonstrating that I can do data science, not data science with silly stories) while job hunting is important for visibility (plus I remember it so much better after writing about it), but at some point it is more fun if I let some silliness in; and if I can add some fun to my life while job hunting during a pandemic I am going to seize opportunity. 

It's also high time (or is it high tide?) someone wrote a data science blog post filled with terrible pirate puns and R code. Please note that these data are fictional, and do not come from any real company. I think of this article both as a portfolio and instructional piece, and therefor I think that the validity of these data are far less important compared to the code, and what I show that we can learn from potential findings.

The scenario I'll be working with for all of the parts of this exercise is as follows: Imagine you discovered the true negative correlation between number of pirates in the world, and global warming. Also imagine that you are unfamiliar with the idea that while correlation might be causation, it might not be. If that was the case, I would recommend you watch the short talk I gave on the topic at a recent data science conference, because we would not want you coming to the erroneous conclusion that you can fight global warming by getting volunteers to take up piracy.
...
6) The level the pirate reached, which is based on how many missions were completed
7) Age
8) Occupation, with options being student, working, retired, or other
9) Whether there is a preference for day missions, night missions, or both
10) A satisfaction rating, which is given by other crew members post mission, and then averaged
11) What US state the pirate hails from
12) Whether any of the pirate's crew members died during a mission
...
I do want to mess up the accuracy of pirateLevel a little bit, so that we can investigate it in the data wrangling blog post. The first step to doing this is creating a vector that is 0 91% of the time, and 1 9% of the time
```{r lvl_edit_v}
change_lvl <-
  sample(c(0, 1),
         nrow(pirates),
...
ids_missionHours_na <-
  pirates %>% select(pirateId) %>% slice_sample(prop = 0.02) %>% pull()
ids_pirateLevel_na <-
  pirates %>% select(pirateId) %>% slice_sample(prop = 0.04) %>% pull()
ids_age_na <-
...
pirates %<>%
  mutate(
    ...
    Mission1 = ifelse(pirateId %in% ids_Mission1_na, NA, Mission1),
    missionLast = ifelse(pirateId %in% ids_missionLast_na, NA, missionLast),
